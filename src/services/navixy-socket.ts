...
export class NavixySocket {
    ...
    constructor(apiBaseUrl: string = '/api/navixy') {
        if (typeof window === 'undefined') {
            this.url = ''; // Server-side safety
            return;
        }

        // For WebSocket, we need to connect DIRECTLY to Navixy, not through the Vite proxy
        // Vite's HTTP proxy doesn't handle WebSocket upgrades reliably
        // If the URL is a local proxy path like /api/navixy, convert to direct Navixy URL

        let wsHost: string;

        if (apiBaseUrl.startsWith('/api/navixy') || apiBaseUrl === '/api/navixy') {
            // Local proxy path - connect directly to Navixy
            wsHost = 'api.navixy.com/v2';
        } else if (apiBaseUrl.startsWith('http')) {
            // Full URL provided - extract host and convert to WS
            wsHost = apiBaseUrl.replace(/^https?:\/\//, '').replace(/\/$/, '');
        } else {
            // Just a hostname
            wsHost = apiBaseUrl.replace(/\/$/, '');
        }

        // Always use secure WebSocket for Navixy
        this.url = `wss://${wsHost}/event/subscription`;

        console.log('[NavixySocket] WebSocket URL:', this.url);
    }

    connect(sessionKey: string, _trackerIds: number[], onUpdate: (states: Record<number, NavixyTrackerState>) => void) {
    ...
    private handleMessage(data: any) {
        ...
        if (data.type === 'event' && data.event === 'state_batch') {
            ...
            if (Object.keys(updates).length > 0 && this.onUpdate) {
                ...
                // Queue for DB persistence (30s buffer)
                syncService.queueStates(updates);
            }
        }
    }
