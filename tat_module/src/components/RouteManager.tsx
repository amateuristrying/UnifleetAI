...
function densityLabel(routeCount: number): string {
    if (routeCount >= 20) return 'Major Artery';
    if (routeCount >= 10) return 'Busy Corridor';
    if (routeCount >= 5) return 'Moderate';
    if (routeCount >= 2) return 'Light';
    return 'Single Route';
}
...
export default function RouteManager() {
    ...
    const stats = useMemo(() => {
        ...
        const avgRouteKm = routes.length > 0
            ? routes.reduce((s, r) => s + (r.estimated_distance_km || 0), 0) / routes.length
            : 0;
        ...
        return {
            ...
            avgRouteKm: Math.round(avgRouteKm).toLocaleString(),
    ...
    // ─── Bifurcated Stats For Selected Route ─────────────────────
    // ─── Bifurcated Stats For Selected Route ─────────────────────
    const bifurcated = useMemo(() => {
        if (!selectedRoute || selectedEdges.length === 0) {
            return { onwardEdges: [], returnEdges: [], onwardDist: 0, returnDist: 0 };
        }

        const finalDestination = selectedRoute.point_c || selectedRoute.point_b;
        let splitIndex = selectedEdges.length;
        for (let i = 0; i < selectedEdges.length; i++) {
            const edge = selectedEdges[i];
            const toNode = edge.to_node;
            if (toNode === finalDestination) {
                splitIndex = i + 1;
                break;
            }
        }

        const onwardEdges = selectedEdges.slice(0, splitIndex);
        const returnEdges = selectedEdges.slice(splitIndex);

        const onwardDist = Math.round(onwardEdges.reduce((s, e) => s + (e.distance_km || 0), 0));
        const returnDist = Math.round(returnEdges.reduce((s, e) => s + (e.distance_km || 0), 0));

        return { onwardEdges, returnEdges, onwardDist, returnDist };
    }, [selectedRoute, selectedEdges]);
        ...
        <div className="relative">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-3 mb-4">
                {[
                    ...
                    { label: 'Avg. Round-Trip', value: `${stats.avgRouteKm} km`, icon: TrendingUp, color: 'text-amber-500' },
                ...
                {selectedRoute && (
                    <div className="w-96 flex-shrink-0 bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden" style={{ maxHeight: 600, overflowY: 'auto' }}>
                        ...
                        <div className="grid grid-cols-2 gap-px bg-gray-100 border-b border-gray-200">
                            {[
                                {
                                    label: 'Total R/T Dist.',
                                    value: selectedEdges.length > 0
                                        ? `${Math.round(bifurcated.onwardDist + bifurcated.returnDist).toLocaleString()} km`
                                        : (selectedRoute.estimated_distance_km ? `${selectedRoute.estimated_distance_km.toLocaleString()} km` : '–'),
                                    icon: TrendingUp
                                },
                                {
                                    label: 'Distance Split',
                                    value: bifurcated.returnEdges.length > 0
                                        ? `${bifurcated.onwardDist.toLocaleString()} / ${bifurcated.returnDist.toLocaleString()} km`
                                        : selectedEdges.length > 0
                                            ? `${bifurcated.onwardDist.toLocaleString()} km`
                                            : (selectedRoute.estimated_distance_km ? `${selectedRoute.estimated_distance_km.toLocaleString()} km` : '–'),
                                    icon: Ruler
                                },
                                {
                                    label: 'Est. Duration',
                                    value: bifurcated.returnEdges.length > 0
                                        ? `${(bifurcated.onwardDist / 50).toFixed(1)}h / ${(bifurcated.returnDist / 50).toFixed(1)}h`
                                        : selectedEdges.length > 0
                                            ? `${(bifurcated.onwardDist / 50).toFixed(1)}h`
                                            : (selectedRoute.estimated_duration_hrs ? `${selectedRoute.estimated_duration_hrs}h` : '–'),
                                    icon: Clock
                                },
                                {
                                    label: 'Segments',
                                    value: bifurcated.returnEdges.length > 0
                                        ? `${bifurcated.onwardEdges.length} / ${bifurcated.returnEdges.length}`
                                        : selectedEdges.length > 0
                                            ? `${bifurcated.onwardEdges.length}`
                                            : '–',
                                    icon: Layers
                                },
                                {
                                    label: 'Corridor Type',
                                    value: CORRIDOR_LABELS[selectedRoute.corridor_type],
                                    icon: Target
                                },
                            ].map(item => (
                                <div key={item.label} className="bg-white px-4 py-3">
                                    ...
                                    <div className="text-sm font-bold text-gray-900 mt-0.5 whitespace-nowrap overflow-hidden text-ellipsis">{item.value}</div>
                                </div>
                            ))}
                        </div>
                            ...
                            {selectedEdges.length > 0 ? (
                                <div className="space-y-4">
                                    ...
                                    <div className="space-y-1">
                                        {bifurcated.returnEdges.length > 0 && (
                                            <h5 className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Onward Path</h5>
                                        )}
                                        {bifurcated.onwardEdges.map((edge, i) => {
                                            const from = edge.from_node;
                                            const to = edge.to_node;
                                            return (
                                                <div key={`${edge.edge_id}-${edge.sequence_order}`} className="flex items-center gap-2 py-2 px-3 bg-gray-50 rounded-lg">
                                        ...
                                        {bifurcated.returnEdges.length > 0 && (
                                            <div className="flex justify-between items-center py-2 px-3 bg-gray-50 border-t border-gray-200 mt-1">
                                                <span className="text-xs font-medium text-gray-600">Onward Subtotal</span>
                                    ...
                                    {bifurcated.returnEdges.length > 0 && (
                                        <div className="space-y-1 border-t border-dashed border-gray-300 pt-3">
                                            <h5 className="text-xs font-bold text-amber-600 uppercase tracking-wider mb-2">Return Path</h5>
                                            {bifurcated.returnEdges.map((edge, i) => {
                                                const from = edge.from_node;
                                                const to = edge.to_node;
                                                return (
                                                    <div key={`${edge.edge_id}-${edge.sequence_order}`} className="flex items-center gap-2 py-2 px-3 bg-gray-50 rounded-lg">
                                                        <div className="w-5 h-5 rounded-full bg-amber-100 text-amber-700 text-xs font-bold flex items-center justify-center flex-shrink-0">
                                                            {bifurcated.onwardEdges.length + i + 1}
                                                        </div>
                                                        <div className="flex-1 min-w-0">
                                                            <div className="text-sm font-medium text-gray-800 truncate">
                                                                {from} → {to}
                                            ...
                                            <div className="flex justify-between items-center py-2 px-3 bg-gray-50 border-t border-gray-200 mt-1">
                                                <span className="text-xs font-medium text-gray-600">Return Subtotal</span>
                                                <span className="text-sm font-bold text-amber-700">
                                                    {bifurcated.returnDist.toLocaleString()} km
                                                </span>
                                            </div>
                                        </div>
                                    )}
                                    ...
                                    <div className="mt-4 p-3 bg-cyan-50 border border-cyan-200 rounded-lg">
                                        <div className="flex justify-between items-center">
                                            <span className="text-xs font-medium text-cyan-700">Total Round-Trip Distance</span>
