...
const sql = `
-- =============================================================
-- ASAS Trip Lifecycle & TAT State Machine - MATERIALIZED VIEW
-- =============================================================
...
tmp_trip_bounds AS (
    SELECT
        ...
        COALESCE(
            (SELECT m.in_time_dt FROM tmp_merged m
             WHERE m.tracker_id = ls.tracker_id
             AND m.geo_level IN ('L1_OFFLOADING', 'L3_DRC_CUSTOMER')
             AND m.in_time_dt > ls.loading_exit
             AND m.in_time_dt < COALESCE(ls.next_loading_entry, 'infinity'::timestamptz)
             ORDER BY m.out_time_dt DESC LIMIT 1),
            (SELECT m.in_time_dt FROM tmp_merged m
             WHERE m.tracker_id = ls.tracker_id
             AND m.geo_level = 'L1_DRC_REGION'
             AND m.in_time_dt > ls.loading_exit
             AND m.in_time_dt < COALESCE(ls.next_loading_entry, 'infinity'::timestamptz)
             ORDER BY m.out_time_dt DESC LIMIT 1),
            (SELECT m.in_time_dt FROM tmp_merged m
             WHERE m.tracker_id = ls.tracker_id AND m.geo_level = 'L1_LOCAL_DELIVERY'
             AND m.in_time_dt > ls.loading_exit
             AND m.in_time_dt < COALESCE(ls.next_loading_entry, 'infinity'::timestamptz)
             AND NOT EXISTS (
                SELECT 1 FROM tmp_merged m2
                WHERE m2.tracker_id = ls.tracker_id
                AND (m2.geo_level LIKE 'L2_BORDER%' OR m2.geo_level IN ('L2_ZAMBIA_CORRIDOR', 'L2_TZ_CORRIDOR'))
                AND m2.in_time_dt > ls.loading_exit
                AND m2.in_time_dt < COALESCE(ls.next_loading_entry, 'infinity'::timestamptz)
             )
             ORDER BY m.out_time_dt DESC LIMIT 1)
        ...
        COALESCE(
            (SELECT m.geofence_name FROM tmp_merged m
             WHERE m.tracker_id = ls.tracker_id AND m.geo_level IN ('L1_OFFLOADING', 'L3_DRC_CUSTOMER')
             AND m.in_time_dt > ls.loading_exit AND m.in_time_dt < COALESCE(ls.next_loading_entry, 'infinity'::timestamptz)
             ORDER BY m.out_time_dt DESC LIMIT 1),
            (SELECT m.geofence_name FROM tmp_merged m
             WHERE m.tracker_id = ls.tracker_id AND m.geo_level = 'L1_DRC_REGION'
             AND m.in_time_dt > ls.loading_exit AND m.in_time_dt < COALESCE(ls.next_loading_entry, 'infinity'::timestamptz)
             ORDER BY m.out_time_dt DESC LIMIT 1),
            (SELECT m.geofence_name FROM tmp_merged m
             WHERE m.tracker_id = ls.tracker_id AND m.geo_level = 'L1_LOCAL_DELIVERY'
             AND m.in_time_dt > ls.loading_exit AND m.in_time_dt < COALESCE(ls.next_loading_entry, 'infinity'::timestamptz)
             AND NOT EXISTS (
                SELECT 1 FROM tmp_merged m2 WHERE m2.tracker_id = ls.tracker_id AND (m2.geo_level LIKE 'L2_BORDER%' OR m2.geo_level IN ('L2_ZAMBIA_CORRIDOR', 'L2_TZ_CORRIDOR'))
                AND m2.in_time_dt > ls.loading_exit AND m2.in_time_dt < COALESCE(ls.next_loading_entry, 'infinity'::timestamptz)
             )
             ORDER BY m.out_time_dt DESC LIMIT 1)
        ) as dest_name,

        COALESCE(
            (SELECT m.out_time_dt FROM tmp_merged m
             WHERE m.tracker_id = ls.tracker_id AND m.geo_level IN ('L1_OFFLOADING', 'L3_DRC_CUSTOMER')
             AND m.in_time_dt > ls.loading_exit AND m.in_time_dt < COALESCE(ls.next_loading_entry, 'infinity'::timestamptz)
             ORDER BY m.out_time_dt DESC LIMIT 1),
            (SELECT m.out_time_dt FROM tmp_merged m
        ...
        COALESCE(
            (SELECT MIN(m.in_time_dt) FROM tmp_merged m WHERE m.tracker_id = ls.tracker_id AND m.geo_level = 'L1_ASAS_BASE'
             AND m.in_time_dt > (SELECT COALESCE((SELECT MAX(m2.out_time_dt) FROM tmp_merged m2 WHERE m2.tracker_id = ls.tracker_id AND m2.geo_level IN ('L1_OFFLOADING', 'L1_DRC_REGION', 'L3_DRC_CUSTOMER', 'L1_LOCAL_DELIVERY') AND m2.in_time_dt > ls.loading_exit AND m2.in_time_dt < COALESCE(ls.next_loading_entry, 'infinity'::timestamptz)), ls.loading_exit + INTERVAL '48 hours'))
             AND m.in_time_dt <= COALESCE(ls.next_loading_entry, 'infinity'::timestamptz)
            ),
            (SELECT MIN(m.in_time_dt) FROM tmp_merged m WHERE m.tracker_id = ls.tracker_id AND m.geo_level = 'L1_LOADING'
             AND m.in_time_dt > (SELECT COALESCE((SELECT MAX(m2.out_time_dt) FROM tmp_merged m2 WHERE m2.tracker_id = ls.tracker_id AND m2.geo_level IN ('L1_OFFLOADING', 'L1_DRC_REGION', 'L3_DRC_CUSTOMER', 'L1_LOCAL_DELIVERY') AND m2.in_time_dt > ls.loading_exit AND m2.in_time_dt < COALESCE(ls.next_loading_entry, 'infinity'::timestamptz)), ls.loading_exit + INTERVAL '48 hours'))
             AND m.in_time_dt <= COALESCE(ls.next_loading_entry, 'infinity'::timestamptz)
            ),
            (SELECT MIN(m.in_time_dt) FROM tmp_merged m WHERE m.tracker_id = ls.tracker_id AND m.geo_level = 'L3_DAR_GATEWAY'
             AND m.in_time_dt > (SELECT COALESCE((SELECT MAX(m2.out_time_dt) FROM tmp_merged m2 WHERE m2.tracker_id = ls.tracker_id AND m2.geo_level IN ('L1_OFFLOADING', 'L1_DRC_REGION', 'L3_DRC_CUSTOMER', 'L1_LOCAL_DELIVERY') AND m2.in_time_dt > ls.loading_exit AND m2.in_time_dt < COALESCE(ls.next_loading_entry, 'infinity'::timestamptz)), ls.loading_exit + INTERVAL '48 hours'))
             AND m.in_time_dt <= COALESCE(ls.next_loading_entry, 'infinity'::timestamptz)
            )
        ) as next_dar_entry
...
CREATE INDEX IF NOT EXISTS tat_trips_view_loading_start_idx ON tat_trips_view(loading_start);
...
BEGIN
    ...
    WHERE dest_exit IS NOT NULL AND next_dar_entry IS NOT NULL AND loading_exit >= p_start_date AND loading_entry <= p_end_date
      AND (p_destination IS NULL OR dest_name = p_destination) AND (p_trip_type IS NULL OR CASE WHEN has_corridor_event THEN 'long_haul' ELSE 'local_ops' END = p_trip_type) AND (p_tracker_id IS NULL OR tracker_id = p_tracker_id);
    
    SELECT COUNT(*) INTO v_total_returning FROM tat_trips_view 
    WHERE dest_exit IS NOT NULL AND next_dar_entry IS NULL AND loading_exit >= p_start_date AND loading_entry <= p_end_date
      AND (p_destination IS NULL OR dest_name = p_destination) AND (p_trip_type IS NULL OR CASE WHEN has_corridor_event THEN 'long_haul' ELSE 'local_ops' END = p_trip_type) AND (p_tracker_id IS NULL OR tracker_id = p_tracker_id);
    ...
    FROM (
        SELECT 
            ...
            CASE WHEN t.has_corridor_event THEN 'long_haul' ELSE 'local_ops' END as trip_type,

            (
                SELECT COALESCE(json_agg(
                    json_build_object(
                        'geofence_name', gv.geofence_name,
                        'in_time', gv.in_time_dt,
                        'out_time', gv.out_time_dt,
                        'event_type', CASE 
                            WHEN gv.geofence_name IN ('TIPER DEPOT', 'Puma Depo Kurasini', 'Oryx Loading Depo (Kigamboni)', 'Oryx Dar Depo', 'Oilcom Dar Depo', 'OILCOM LIMITED TERMINAL DEPOT', 'MERU TERMINAL DEPOT', 'MOGAS OIL DEPOT', 'SUPERSTAR FUEL DEPOT', 'GBP DRS DEPOT', 'ORYX FUEL DEPOT', 'WORLD OIL DEPOT', 'Tanga GF', 'Mtwara GF', 'Beira', 'KURASINI ALL TOGETHER') THEN 'loading'
                            WHEN gv.geofence_name IN ('LUSAKA DEPOT', 'Ndola Offloading', 'Mzuzu Offloading', 'Lilongwe', 'EXPREE OIL DEPOT', 'SEP CONGO', 'Sep Congo', 'United Petroleum Lubumbashi', 'KANATA PETROLEUM DEPOT (CONSTALINA)', 'Kolwezi Offloading', 'LUALABA OIL (KOLWEZI)', 'United Petroleum Kolwezi', 'Frontier') THEN 'unloading'
                            WHEN gv.geofence_name IN ('TUNDUMA BORDER TZ SIDE', 'Tanzania Tunduma Border', 'NAKONDE BORDER ZMB SIDE', 'Zambia Nakonde Border', 'Tunduma Border 1', 'KASUMBALESA ZMB SIDE', 'SAKANIA ZMB SIDE', 'Sakania border', 'KASUMBALESA BORDER  DRC SIDE', 'Kasumbalesa Border (DRC)', 'KASUMBALESA', 'SAKANIA DRC', 'Mokambo border', 'KASUMULU BORDER', 'CHIRUNDU BORDER', 'CHIRUNDU BORDER ZIM SIDE', 'CHIRUNDU BORDER ZAMBIA SIDE', 'KABANGA BORDER', 'RUSUMO BORDER', 'MALABA BORDER', 'Horohoro border', 'MUTUKULA BORDER') THEN 'border'
                            ELSE 'transit'
                        END
        ...
        WHERE t.loading_exit >= p_start_date AND t.loading_entry <= p_end_date
          ...
          AND (p_trip_type IS NULL OR CASE WHEN t.has_corridor_event THEN 'long_haul' ELSE 'local_ops' END = p_trip_type)
...
fs.writeFileSync('supabase/migrations/tat_optimization_mv_only.sql', sql + getDetailsFunc);
