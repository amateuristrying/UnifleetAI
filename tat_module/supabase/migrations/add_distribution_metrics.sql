-- Add distribution metrics to get_stop_patterns RPC
-- Adds: p10, p25, median (p50), p75, min, max, stddev for visit duration
-- These enable percentile-based insights in the Efficiency tooltip

-- Must drop first because we're adding new OUT parameters (changing return type)
...
-- Force PostgREST to reload schema cache so the new columns are visible
