-- V2 FORCE FIX: Performance Optimization for Stitching
-- 1. Adds "Noise Filter" (ignore stops < 5 mins) to drastically reduce row count for window functions.
-- 2. Forces recreation of the optimized index.
-- 3. Sets timeout to 300s.

-- A. Force Index Recreation
...
-- B. Update Function with Noise Filter
...
BEGIN
    ...
    WITH filtered_stops AS (
        ...
        WHERE srs.stop_start >= min_date
          ...
          -- NOISE FILTER: Ignore stops shorter than 5 minutes (0.083 hours)
          -- This removes traffic lights/congestion checks from the "Bottleneck" analysis
          -- and makes the query MUCH faster.
          AND srs.stop_duration_hours > 0.083 
