-- =============================================================
-- TAT V2 REFACTOR: Phase 12 — Phase 3 DAR Anchor Fix
-- Purpose:
--   Force replace build_trip_state_events_v2() with corrected
--   trip_anchor_start selection:
--   - anchor lower bound = max(previous destination exit, previous loading start)
--   - choose earliest event in latest contiguous pre-loading origin session
-- Dependency: tat_v2_refactor_phase_11_phase3_session_overlap_fix.sql (or phase_3_fix)
--
-- Changes vs currently deployed phase_11:
...
--   [DAR_ANCHOR] trip_anchor_start no longer back-anchors to stale daily
--                origin rows from prior trips.
-- =============================================================
...
BEGIN
    ...
    WHERE el.trip_key <> tw.trip_key
      ...
      -- Guardrail: allow deeper stale-key cleanup. 45 days was too short for
      -- legacy keys that leaked across long idle/return cycles.
    ...
    JOIN LATERAL (
        ...
        WHERE n.tracker_id = tw.tracker_id
          ...
          AND n.in_time    < tw.window_end
          AND (
                n.role_code IN ('origin_zone','origin_gateway')
                OR (
                    n.role_code IN ('ops_yard','origin_base')
