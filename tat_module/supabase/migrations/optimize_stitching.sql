-- Optimization for Stop Pattern Stitching
-- specific index for window functions (PARTITION BY tracker_id ORDER BY stop_start)

CREATE INDEX IF NOT EXISTS idx_stop_risk_scores_tracker_start 
ON stop_risk_scores (tracker_id, stop_start);

-- Re-apply function with slightly increased timeout just in case (already 120s but confirming)
...
AS $$
BEGIN
    -- Increase timeout even further for large history stitching (5 minutes)
    ...
    site_visits AS (
        ...
        SELECT
            -- Use the H3 index of the FIRST stop in the session (or mode?)
            -- Simple: Max/Min H3 might behave weirdly. 
            -- Better: First H3 Index for stability.
