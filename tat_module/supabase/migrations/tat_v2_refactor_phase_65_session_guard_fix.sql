-- =============================================================
-- TAT V2 REFACTOR: Phase 65
-- Loading Session Guard Fix — `bool_and` → `MAX(end)::time` check
--
-- Bug:
--   Phase 62 used:
--     bool_and(NOT (is_open_geofence OR visit_end_utc::time = '23:59:59'))
--   as the midnight guard in the loading session rollup.
--
--   This is WRONG because bool_and evaluates across EVERY visit in
--   the session group. A loading session that spans multiple calendar
--   days will always contain intermediate rows ending at 23:59:59
--   (the daily data ceiling), so bool_and always returns FALSE for any
--   multi-day session. The result: session_out = NULL for every session
--   that crosses midnight — even when the truck clearly finished loading.
--
--   Cascade:
--     session_out = NULL
--     → loading_end = NULL in _trip_context
--     → all LATERAL JOINs use (visit_start_utc > NULL) → evaluates NULL
--     → no destination / border / return events found
--     → entire trip is orphaned with only a loading_start event
--     → facts builder drops it (low confidence / no destination)
--
-- Fix:
--   The correct question is: "does the LAST exit in this session land
--   at midnight?" — not "does ANY exit land at midnight?"
--
--   Replace with:
--     NOT bool_or(s.is_open_geofence)
--     AND MAX(s.visit_end_utc)::time != '23:59:59'::time
--
--   MAX(visit_end_utc) = the latest actual end timestamp in the session.
--   If that is not 23:59:59, the session has a real physical end.
--   bool_or(is_open_geofence) still correctly catches open sessions.
--
-- Only change vs Phase 62: lines 107-110 in the rollup CTE.
...
BEGIN
    ...
    VALUES (
        'PHASE_65_SESSION_GUARD', 'running',
    ...
    rollup AS (
        SELECT
            ...
            -- OLD (Phase 62 — buggy):
            --   bool_and(NOT (is_open_geofence OR visit_end_utc::time = '23:59:59'))
            --   → FALSE for ANY multi-day session (intermediate rows hit midnight)
            --   → session_out = NULL → entire trip orphaned
            --
            -- NEW (Phase 65 — correct):
            --   Check only the MAXIMUM (final) visit_end_utc of the session.
            --   If the last exit is not a midnight boundary AND no geofence is
            --   still open, the session has a real physical end.
            -- ────────────────────────────────────────────────────────────────
            CASE
    ...
    WHERE ls.session_out IS NOT NULL; -- Midnight Guard (session still in progress → skip)
    ...
    LEFT JOIN LATERAL (
        ...
        WHERE ov.tracker_id = tw.tracker_id
          ...
          AND ov.visit_start_utc > COALESCE(
                dse.dest_exit, dre.dest_region_exit,
    ...
    SELECT
        ...
        tc.dest_name, 'destination_site', 'destination'
    FROM _trip_context tc
    WHERE tc.dest_exit IS NOT NULL; -- Already guarded in CTE (NOT midnight)
    ...
    PERFORM set_config('tat.current_rule_version', 'phase65_v1', true);
