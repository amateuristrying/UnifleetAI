-- =============================================================
-- TAT V2 REFACTOR: Phase 76b
-- Temporal Continuity (coverage_start) Fix
--
-- This migration performs a full rebuild of build_trip_state_events_v2
-- to integrate the coverage_start logic successfully.
--
-- Rebuild prevents fragile text-REPLACE issues encountered in Phase 76.
--
-- Logic:
--   1. _trip_windows gets coverage_start = last_trip_closed_ts (if < session_in).
--   2. pre_origin search and dar_arrival search use coverage_start as left boundary.
--   3. All Phase 76 fixes (gateway exclusion, dynamic roles) are preserved.
-- =============================================================
...
BEGIN
    ...
    WITH loading_only AS (
        ...
        -- Branch A: Terminal-level loading stops
        SELECT ov.*
        FROM _ops_visits ov
        ...
        -- Branch B: Origin-zone loading hub extension (Kurasini)
        SELECT ov.*
        FROM _ops_visits ov
    ...
    CREATE TEMP TABLE _trip_windows (
        ...
        window_start       TIMESTAMPTZ,   -- = session_in
        window_end         TIMESTAMPTZ,   -- = next session_in or infinity
        prev_window_start  TIMESTAMPTZ,   -- = previous session_in
        coverage_start     TIMESTAMPTZ    -- Phase 76b: temporal_continuity
    ) ON COMMIT DROP;
    ...
    SELECT
        ...
        -- Phase 76b: coverage_start
        -- Coverage starts at the exact millisecond the previous trip closed.
        -- If no previous trip found in current state events, fallback to session_in.
        GREATEST(
            COALESCE(
                (
                    SELECT MAX(e.event_time)
                    FROM public.trip_state_events e
                    WHERE e.tracker_id = s.tracker_id
                      AND e.event_code = 'trip_closed'
                      AND e.event_time < s.session_in
                ),
                s.session_in - INTERVAL '30 days'
            ),
            s.session_in - INTERVAL '30 days' -- Basic lookback
        ) AS coverage_start
    FROM sequenced s
    ...
    JOIN _loading_sessions ls
      ON ls.tracker_id = tw.tracker_id AND ls.session_in = tw.window_start
    WHERE ls.session_out IS NOT NULL;

    -- 6) Compute state-machine context per trip
    CREATE TEMP TABLE _trip_context ON COMMIT DROP AS
    SELECT
        tw.trip_key, tw.tracker_id, tw.tracker_name, tw.loading_terminal,
        ...
        pre.pre_origin_in, pre.pre_origin_name,
        ox.origin_exit, ox.origin_exit_name,
        ds.dest_entry, dse.dest_exit, ds.dest_name,
        dr.dest_region_entry, dre.dest_region_exit, dr.dest_region_name,
        CASE
        ...
        ro.return_origin_entry, ro.return_origin_name
    FROM _trip_windows tw
    LEFT JOIN _loading_sessions ls ON ls.tracker_id = tw.tracker_id AND ls.session_in = tw.window_start
    LEFT JOIN LATERAL (
        WITH pre_origin AS (
            SELECT
                ov.*,
            ...
            WHERE ov.tracker_id = tw.tracker_id
              ...
              -- Phase 76b: coverage_start left boundary
              AND ov.visit_start_utc >= COALESCE(tw.coverage_start, tw.prev_window_start, tw.window_start - INTERVAL '30 days')
        ),
        sessioned AS (
            SELECT p.*, SUM(CASE WHEN p.prev_max_out IS NULL THEN 1 WHEN p.prev_max_out >= p.visit_start_utc - INTERVAL '6 hours' THEN 0 ELSE 1 END) OVER (ORDER BY p.visit_start_utc, p.visit_end_for_overlap_utc) AS session_group FROM pre_origin p
        ),
    ...
    LEFT JOIN LATERAL (
        SELECT MIN(ov.visit_start_utc) AS first_signal FROM _ops_visits ov 
        WHERE ov.tracker_id = tw.tracker_id AND ov.visit_start_utc > ls.session_out AND ov.visit_start_utc < tw.window_end
        AND ov.stop_state IN ('corridor_transit', 'border_crossing', 'customs_stop', 'destination_stop', 'destination_region_presence')
    ) sig ON true
    LEFT JOIN LATERAL (
        SELECT ov.visit_end_for_overlap_utc AS origin_exit, ov.geofence_name AS origin_exit_name FROM _ops_visits ov
        WHERE ov.tracker_id = tw.tracker_id AND ov.stop_state = 'origin_operational_stop' AND ov.visit_start_utc >= ls.session_out
        AND ov.visit_start_utc < COALESCE(sig.first_signal, tw.window_end) AND NOT (ov.is_open_geofence OR ov.visit_end_utc::time = '23:59:59'::time)
        ORDER BY ov.visit_end_for_overlap_utc DESC LIMIT 1
    ) ox ON true
    LEFT JOIN LATERAL (
        SELECT ov.visit_start_utc AS dest_entry, ov.geofence_name AS dest_name FROM _ops_visits ov
        WHERE ov.tracker_id = tw.tracker_id AND ov.stop_state = 'destination_stop' AND ov.visit_start_utc > ls.session_out AND ov.visit_start_utc < tw.window_end
        ORDER BY ov.visit_start_utc ASC LIMIT 1
    ) ds ON true
    LEFT JOIN LATERAL (
        SELECT MAX(ov.visit_end_for_overlap_utc) AS dest_exit FROM _ops_visits ov
        WHERE ov.tracker_id = tw.tracker_id AND ov.stop_state = 'destination_stop' AND ds.dest_entry IS NOT NULL
        AND ov.visit_start_utc >= ds.dest_entry AND ov.visit_start_utc < tw.window_end AND NOT (ov.is_open_geofence OR ov.visit_end_utc::time = '23:59:59'::time)
    ) dse ON true
    LEFT JOIN LATERAL (
        SELECT ov.visit_start_utc AS dest_region_entry, ov.geofence_name AS dest_region_name FROM _ops_visits ov
        WHERE ov.tracker_id = tw.tracker_id AND ov.stop_state = 'destination_region_presence' AND ov.visit_start_utc > ls.session_out AND ov.visit_start_utc < tw.window_end
        ORDER BY ov.visit_start_utc ASC LIMIT 1
    ) dr ON true
    LEFT JOIN LATERAL (
        SELECT MAX(ov.visit_end_for_overlap_utc) AS dest_region_exit FROM _ops_visits ov
        WHERE ov.tracker_id = tw.tracker_id AND ov.stop_state = 'destination_region_presence' AND dr.dest_region_entry IS NOT NULL
        AND ov.visit_start_utc >= dr.dest_region_entry AND ov.visit_start_utc < tw.window_end AND NOT (ov.is_open_geofence OR ov.visit_end_utc::time = '23:59:59'::time)
    ) dre ON true
    LEFT JOIN LATERAL (
        SELECT ov.visit_start_utc AS return_origin_entry, ov.geofence_name AS return_origin_name FROM _ops_visits ov
        WHERE ov.tracker_id = tw.tracker_id AND ov.stop_state IN ('origin_operational_stop', 'origin_loading_stop')
        AND ov.visit_start_utc > COALESCE(dse.dest_exit, dre.dest_region_exit, ds.dest_entry, dr.dest_region_entry, ls.session_out)
        AND ov.visit_start_utc < tw.window_end
        ORDER BY ov.visit_start_utc ASC LIMIT 1
    ) ro ON true;

    -- Steps 7-12: Insert rest of events from _trip_context (Phase 75 Priority code included)
    ...
    INSERT INTO public.trip_state_events (trip_key, tracker_id, tracker_name, event_code, event_time, event_confidence, inference_rule, event_meta, canonical_name, role_code, trip_stage)
    SELECT tc.trip_key, tc.tracker_id, tc.tracker_name, 'corridor_entry', ov.visit_start_utc, 0.80, 'state_machine_corridor', jsonb_build_object('geofence', ov.geofence_name), ov.geofence_name, 'corridor_region', 'transit'
    FROM _trip_context tc JOIN _ops_visits ov ON ov.tracker_id = tc.tracker_id AND ov.stop_state = 'corridor_transit' AND ov.visit_start_utc > tc.loading_end AND ov.visit_start_utc < COALESCE(tc.first_destination_signal, tc.window_end);
    ...
    PERFORM set_config('tat.current_rule_version', 'phase76b_v1', true);
